...
if REPORT_TYPE == "daily":
    day_timestamp_range = module_general_funcs.day_timestamp_range_from_date(date)
    start_timestamp = day_timestamp_range[0]
    end_timestamp = day_timestamp_range[1]

elif REPORT_TYPE == "weekly":
    week_timestamp_range = module_general_funcs.week_timestamp_range_from_date(date)
    ...
    start_date = module_general_funcs.timestamp_to_date(start_timestamp).strftime('%Y_%m_%d')
    end_date = module_general_funcs.timestamp_to_date(end_timestamp).strftime('%Y_%m_%d')

elif REPORT_TYPE == "monthly":
    month_timestamp_range = module_general_funcs.month_timestamp_range_from_date(date)
...
for department, extension in DEPARTMENTS.items():
    print("\n\n\n----->{}".format(department))
    cdr_records = module_cdr_funcs.get_cdr_by_called_number(start_timestamp, end_timestamp, extension)
    callTreeStats = cdr_classes.CallTreeStats(extension, department)
    module_cdr_funcs.count_calls_by_call_tree(callTreeStats, cdr_records)
...
for department, extension in EXTENSIONS.items():
    print("\n\n\n----->{}".format(department))
    cdr_records = module_cdr_funcs.get_cdr_by_called_number(start_timestamp, end_timestamp, extension)
    extensionStats = cdr_classes.ExtensionStats(extension, department)
    module_cdr_funcs.count_calls_by_extension(extensionStats, cdr_records)
...
for department, extension in HUNTPILOTS.items():
    cdr_records = module_cdr_funcs.get_cdr_by_called_number(start_timestamp, end_timestamp, extension)
    huntpilotStats = cdr_classes.HuntPilotStats(extension, department)
    module_cdr_funcs.count_calls_by_hunt_pilot(huntpilotStats, cdr_records)
